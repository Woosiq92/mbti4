...
export default function TestPageClient() {
  ...
  const goNext = () => {
    ...
    if (sasangStep === TOTAL_QUESTIONS - 1) {
      ...
      router.push(
        `/result?mbti=${encodeURIComponent(mbtiToSave)}&sasang=${encodeURIComponent(
          sasangName
        )}`
      );
      return;
    ...
    <main className="min-h-screen">
      <div className="mx-auto flex max-w-2xl flex-col gap-6 px-4 pb-12 pt-10">
        <header className="space-y-2">
          <h1 className="text-2xl font-bold text-slate-900">테스트</h1>
          <p className="text-sm text-slate-600">
            {step === 1 ? "Step 1: MBTI 선택" : `Step 2: 사상체질 검사 (${sasangStep + 1} / ${TOTAL_QUESTIONS})`}
          </p>
        </header>

        {step === 1 ? (
          <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <h2 className="text-base font-semibold text-slate-900">Step 1: MBTI 선택</h2>
            <p className="mt-2 text-sm leading-relaxed text-slate-600">
              잘 모르겠다면 &lsquo;모름&rsquo;을 선택해도 됩니다. 사상체질 결과만 먼저 확인할 수 있어요.
            </p>
            <p className="mt-2 text-sm leading-relaxed text-slate-600">
              본 검사는 의학적 진단이 아니며, 사용자의 자기선택을 기반으로 결과를 조합합니다.
            </p>
          ...
          <>
            <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  <p className="text-sm font-semibold text-slate-900">질문 {sasangStep + 1}</p>
                  <p className="mt-1 text-sm leading-relaxed text-slate-700">{currentQuestion.question}</p>
                </div>
              ...
              <p className="mt-4 text-sm text-slate-600">선택한 답변은 다음/이전 문항 이동 시 유지됩니다.</p>
            </section>
            ...
            <section className="rounded-2xl border border-amber-100 bg-amber-50 p-4">
              <p className="text-sm leading-relaxed text-amber-900/90">
                본 검사는 자기이해와 재미를 위한 콘텐츠이며, 의학적 진단이 아닙니다.
              </p>
            </section>
          </>
        )}
