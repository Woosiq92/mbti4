...
const EVIDENCE_FALLBACK =
  "당신의 응답을 함께 살펴봤을 때 한쪽으로 단정하기는 어려워 보였어요. 아래 내용은 전체 흐름을 가볍게 정리한 참고용입니다.";

const REACTION_SPEED_BY_KEY: Record<PairKey, string> = {
  AA: "당신의 응답에서는 새 일을 시작할 때 먼저 움직이는 편이고, 반응과 표현이 비교적 직접적으로 보였어요.",
  BB: "당신의 응답에서는 충분히 생각한 뒤 움직이는 쪽에 가깝고, 반응과 표현도 조심스러운 흐름이 더 강했어요.",
  AB: "당신의 응답에서는 움직임은 빠르게 시작하지만, 표현은 조심스럽게 가져가는 경향이 함께 보였어요.",
  BA: "당신의 응답에서는 생각을 정리한 뒤 움직이는 편이면서도, 함께할 때는 비교적 바로 반응하는 모습도 보였어요.",
};

const ENERGY_DIRECTION_BY_KEY: Record<PairKey, string> = {
  AA: "당신의 응답에서는 에너지가 바깥으로 드러나기 쉬운 편에 가깝고, 전체 분위기도 가볍게 위로 뻗는 느낌으로 읽혔어요.",
  BB: "당신의 응답에서는 에너지를 안으로 쌓아 두는 편에 가깝고, 분위기도 묵직하게 안으로 모이는 쪽이 더 또렷했어요.",
  AB: "당신의 응답에서는 밖으로 드러내는 면과 조용히 쌓아 두는 면이 함께 있었고, 전체 흐름은 바깥으로 퍼지는 쪽에 조금 더 가까웠어요.",
  BA: "당신의 응답에서는 안으로 쌓는 편에 가깝게 응답한 부분이 있으면서도, 분위기는 위로 뻗는 인상이 함께 보였어요.",
};

const STRESS_PATTERN_BY_KEY: Record<PairKey, string> = {
  AA: "당신의 응답에서는 평소 컨디션에서 열감을 느끼기 쉬운 편으로 응답했고, 스트레스가 올라올 때 예민함이나 답답함이 먼저 느껴질 수 있다는 흐름이 더 많았어요.",
  BB: "당신의 응답에서는 몸을 따뜻하게 해야 편한 쪽에 가깝게 응답했고, 압박이 커질수록 기운이 떨어지거나 위축되는 흐름이 더 강하게 보였어요.",
  AB: "당신의 응답에서는 평소에는 열감이나 활발한 반응이 느껴지지만, 스트레스 상황에서는 소진이나 위축이 먼저 올라온다고 느끼는 흐름도 함께 보였어요.",
  BA: "당신의 응답에서는 추위나 안정 쪽 컨디션에 가깝게 응답한 편이면서도, 스트레스에서는 예민함이나 답답함이 먼저 느껴질 수 있다는 흐름도 함께 있었어요.",
};

const LIFE_RHYTHM_BY_KEY: Record<PairKey, string> = {
  AA: "당신의 응답에서는 활동량이 늘면 오히려 살아나는 편에 가깝고, 생활 리듬도 빠르게 몰입하는 쪽으로 보였어요.",
  BB: "당신의 응답에서는 활동이 쌓이면 지치기 쉬운 편에 가깝고, 일정하고 안정적인 흐름을 더 선호하는 쪽으로도 읽혔어요.",
  AB: "당신의 응답에서는 빠른 생활 리듬에 익숙한 면이 있으면서도, 활동이 겹치면 무리가 쌓인다고 느끼는 흐름도 함께 있었어요.",
  BA: "당신의 응답에서는 몸이 빠른 몰입이 가능한 편으로 보이기도 했지만, 장기적으로는 안정적인 루틴을 더 편하게 느끼는 경향도 보였어요.",
};
...
function getMetricSummary(key: EvidenceMetric["key"], score: number): string {
  ...
  if (key === "reactionSpeed") {
    if (isLeft)
      return "당신의 응답에서는 빠르게 나서기보다 생각을 정리한 뒤 반응하는 흐름이 조금 더 강하게 보였어요.";
    if (isRight)
      return "당신의 응답에서는 먼저 반응하고 움직이는 속도 쪽이 조금 더 강하게 나타났어요.";
    if (score === 50)
      return "당신의 응답에서는 빠른 반응성과 신중한 판단이 함께 보이는 중간형에 가까웠어요.";
    return midLeft
      ? "당신의 응답에서는 전반적으로 생각을 정리한 뒤 반응하는 쪽에 조금 더 가까웠어요."
      : "당신의 응답에서는 전반적으로 빠른 반응 쪽에 조금 더 가까웠어요.";
  }

  if (key === "energyDirection") {
    if (isLeft)
      return "당신의 응답에서는 에너지를 바깥으로 크게 드러내기보다 안으로 쌓고 유지하는 편에 조금 더 가까웠어요.";
    if (isRight)
      return "당신의 응답에서는 에너지를 바깥으로 크게 드러내며 확장하는 흐름이 조금 더 강하게 보였어요.";
    if (score === 50)
      return "당신의 응답에서는 바깥으로 드러내는 면과 안으로 유지하는 면이 함께 나타나는 흐름에 가까웠어요.";
    return midLeft
      ? "당신의 응답에서는 전반적으로 에너지를 안으로 축적하는 쪽에 조금 더 가까웠어요."
      : "당신의 응답에서는 전반적으로 에너지를 바깥으로 발산하는 쪽에 조금 더 가까웠어요.";
  }

  if (key === "stressPattern") {
    if (isLeft)
      return "당신의 응답에서는 압박이 커질수록 예민하게 과열되기보다 기운이 떨어지고 위축되는 흐름이 조금 더 강하게 보였어요.";
    if (isRight)
      return "당신의 응답에서는 압박이 커질수록 예민함이나 열감이 먼저 올라오는 흐름이 조금 더 강하게 나타났어요.";
    if (score === 50)
      return "당신의 응답에서는 스트레스 상황에서 과열과 소진이 함께 흔들리는 중간형에 가까웠어요.";
    return midLeft
      ? "당신의 응답에서는 소진·위축 쪽 경향이 조금 더 가까웠어요."
      : "당신의 응답에서는 과열·예민 쪽 경향이 조금 더 가까웠어요.";
  }

  // lifeRhythm
  if (isLeft)
    return "당신의 응답에서는 순간적인 속도감보다 일정하고 안정적인 리듬을 선호하는 경향이 조금 더 강하게 드러났어요.";
  if (isRight)
    return "당신의 응답에서는 활동량이 늘면 오히려 리듬이 살아나며 빠르게 몰입하는 편에 조금 더 가까웠어요.";
  if (score === 50)
    return "당신의 응답에서는 안정적인 리듬과 빠른 몰입이 함께 나타나는 흐름에 가까웠어요.";
  return midLeft
    ? "당신의 응답에서는 전반적으로 안정적인 흐름을 더 편하게 느끼는 쪽에 조금 더 가까웠어요."
    : "당신의 응답에서는 전반적으로 빠르게 몰입하는 쪽에 조금 더 가까웠어요.";
}
